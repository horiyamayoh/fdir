...
def sql(model: dict[str, Any]) -> str:
    expected_identities = {
        ...
        "InformationUnit": "unitId",
        "RecordAssertion": "assertionId",
        "InformationRelation": "relationId",
        "InventoryDomain": "inventoryDomainId",
        "AccountingItem": "accountingItemId",
        "GuaranteeStatus": "guaranteeStatusId",
        "Diagnostic": "diagnosticId",
    }
    entities = {item["name"]: item for item in model["entities"]}
    for entity_name, identity in expected_identities.items():
        if entities.get(entity_name, {}).get("identity") != identity:
            raise ValueError(
                f"SQLite projection identity mismatch for {entity_name}: {identity}"
            )

    return """-- Generated from machine/logical-model.yaml. Rebuildable projection only; never authority.
